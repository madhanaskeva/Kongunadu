...
import { ChevronDown, Menu, Volume2, Bell, User, LogOut } from 'lucide-react';

export const AdminHeader = ({ onOpenNav, narrow }) => {
  const navigate = useNavigate();
  const { pathname } = useLocation();
  const { user, logout } = useAuth();
  const { setDrawer, setForm, T, adminNotifOpen, setAdminNotifOpen } = useTMSAdmin();
  ...
  const openSendNotice = () => {
    ...
    setForm({ title: '', body: '', branch: 'all', priority: 'Normal' });
    setDrawer({
      title: 'Send notice to supervisors',
      kicker: 'Broadcast',
      ...
      required: ['title', 'body'],
      fields: [
        ['branch', 'Target branch', [['all', 'All branches'], ...branches.map(b => [b.id, b.name])]],
        ['priority', 'Priority', ['Normal', 'Urgent']],
        ['title', 'Notice title', null, 'e.g. Rate revision from 15 Sep'],
        ['body', 'Message body', 'textarea', 'Message text visible to supervisors'],
      ],
    });
  };
  ...
  return (
    <header className="tms-topbar">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 'none' }}>
        <button
          type="button"
          onClick={openSendNotice}
          className="tms-send-notice-btn"
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: '8px',
        ...
        <button
          type="button"
          onClick={() => setAdminNotifOpen && setAdminNotifOpen(!adminNotifOpen)}
          aria-label="Notifications"
          className="tms-topbar-icon-btn"
          style={{
            all: 'unset',
            cursor: 'pointer',
            position: 'relative',
